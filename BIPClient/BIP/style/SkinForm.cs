...
{
    public delegate void IconClickHandle(object sender, EventArgs e);

    public class SkinForm : BipForm
    {
        public event IconClickHandle IconClick = null;
     ...
     /// </summary>
        ...
        private void Init()
        {
           ...
           //初始化的时候把透明度调为0，降低页面加载因为透明度改变的问题而导致闪烁
            double value = 100;
            if (Temp.Open == "True")
            {
                value = double.Parse(Temp.Opacity);
               this.Opacity = (100 - value) / 100.0;
            }
            else
            {
                this.Opacity = 1;
