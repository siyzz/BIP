...
namespace com.ccf.bip.framework.form.control
{
	...
	public class ComboBoxTree : UserControl
	{
        ...
        public string Value
        {
            ...
            {
                _value = value;
                TreeNode node = FindNode(this.tvTreeView, _value);
                if (node != null)
                {
                    Text = node.Text;
                }
        ...
        private TreeNode FindNode(TreeView tree, string name)
        {
            TreeNode node = null;
        ...
        private TreeNode FindNode(TreeNode tnParent, string name)
        {
            if (tnParent == null) return null;
            if (tnParent.Name == name) return tnParent;
		...
		{
            tbSelectedValue.Text = this.tvTreeView.SelectedNode.Text;//this.tvTreeView.SelectedNode.FullPath.Replace(@"\", this._branchSeparator);
            this._value = this.tvTreeView.SelectedNode.Name;
            this.ToggleTreeView(sender, null);
            this.Value = tvTreeView.SelectedNode.Name;
		...
		private void ComboBoxTree_Layout(object sender, System.Windows.Forms.LayoutEventArgs e)
		{
			...
			this.btnSelect.Location = new Point(this.Width - this.btnSelect.Width - 4, 2);

			this.tbSelectedValue.Location = new Point(2, 2);
			this.tbSelectedValue.Width = this.Width - this.btnSelect.Width - 4;

            this.frmTreeView.Size = new Size(150,200);//(this.Width, this.tvTreeView.Height);
			this.pnlTree.Size = this.frmTreeView.Size;
